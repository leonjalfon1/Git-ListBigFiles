...
tmp_dir=${__dir}/ListBigFilesResults
mkdir -p ${tmp_dir}

cd ${THE_DIR}

git rev-list --objects --all | sort -k 2 > ${tmp_dir}/allfileshas.txt
git gc && git verify-pack -v objects/pack/pack-*.idx | egrep "^\w+ blob\W+[0-9]+ [0-9]+ [0-9]+$" | sort -k 3 -n -r > ${tmp_dir}/bigobjects.txt

[[ -f ${tmp_dir}/results.txt ]] && rm ${tmp_dir}/results.txt

for SHA in `cut -f 1 -d\  < ${tmp_dir}/bigobjects.txt | head -n ${FILES_TO_SHOW}`; do
	echo $(grep $SHA ${tmp_dir}/bigobjects.txt) $(grep $SHA ${tmp_dir}/allfileshas.txt) | awk '{print $1,$3,$7}' >> ${tmp_dir}/results.txt
done;

echo ""
echo "=============================================================="
echo "Please see the file:"
echo "   ${tmp_dir}/results.txt"
echo "=============================================================="
